...
fn find_runtime_module_library(module_root: &Path) -> Option<PathBuf> {
    ...
    collect_shared_libraries(&release_dir, 3, &mut libraries);
    libraries.sort();

    if let Some(project_hint) = project_hint {
        if let Some(path) = libraries.iter().find(|path| {
            path.file_name()
