...
use crate::gui::workspace_model::{order_plugins_for_execution, WorkspaceDefinition};

use crate::gui::runtime_bridge::connection_cache::{build_connection_cache, RuntimeConnectionCache};
use crate::gui::runtime_bridge::message_handler::{LogicMessage, LogicSettings, LogicState};
use crate::gui::runtime_bridge::plugin_manager::{
    set_dynamic_config_patch, DynamicPluginInstance, RuntimePlugin,
};
#[cfg(feature = "comedi")]
use crate::gui::runtime_bridge::plugin_processors::process_comedi_daq;
use crate::gui::runtime_bridge::plugin_processors::{
    process_csv_recorder, process_dynamic_plugin, process_live_plotter, process_performance_monitor,
};
use crate::gui::runtime_bridge::rt_thread::ActiveRtBackend;
                ...
                Ok(message) => match message {
                    ...
                    LogicMessage::QueryPluginBehavior(kind, library_path, response_tx) => {
                        let behavior = match kind.as_str() {
                            ...
                            #[cfg(feature = "comedi")]
                            "comedi_daq" => {
                                Some(crate::gui::builtin_tools::comedi_daq::ComediDaqPlugin::new(0).behavior())
                            }
                    ...
                    LogicMessage::QueryPluginMetadata(library_path, response_tx) => {
                            ...
                            unsafe { DynamicPluginInstance::load(&library_path, 0) }
                        {
                            ...
                            let ui_schema: Option<crate::gui::tool_api::ui::UISchema> = if let Some(
                                ui_schema_fn,
                            ) =
