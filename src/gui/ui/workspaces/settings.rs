...
use super::*;
use crate::gui::runtime_bridge::LogicSettings;
use crate::gui::tool_model::workspace::{RUNTIME_MIN_FREQUENCY_VALUE, RUNTIME_MIN_PERIOD_VALUE};
...
impl GuiApp {
    ...
    /// settings including timing parameters (frequency/period), thread priority,
    /// It provides real-time validation and
    ...
    ///
    /// # Side Effects
    /// - Renders fixed-size window with settings controls
    ...
    pub(crate) fn render_workspace_settings_window(&mut self, ctx: &egui::Context) {
        ...
        let window_size = egui::vec2(420.0, 180.0);
        ...
        let mut draft = self
            ...
            .unwrap_or(WorkspaceSettingsDraft {
                ...
                deadline_tolerance_ns: self.deadline_tolerance_ns,
            });
        ...
        let response = egui::Window::new("Runtime settings")
            ...
            .show(ctx, |ui| {
                ...
                ui.add_space(8.0);
                draft.thread_priority = draft.thread_priority.clamp(0, 99);
                ui.horizontal(|ui| {
                    ui.label("Thread priority");
                    ui.add(
                        egui::DragValue::new(&mut draft.thread_priority)
                            .speed(1.0)
                            .clamp_range(0..=99),
                    );
                });
                ui.horizontal(|ui| {
                    ui.label("Deadline tolerance");
                    ui.add(
                        egui::DragValue::new(&mut draft.deadline_tolerance_ns)
                            .speed(100.0)
                            .suffix(" ns"),
                    );
                });
        ...
        if apply_clicked || save_clicked {
            ...
            self.thread_priority = draft.thread_priority.clamp(0, 99);
            self.deadline_tolerance_ns = draft.deadline_tolerance_ns;
            ...
            let daemon_priority_updated =
                match Self::set_daemon_runtime_priority(self.thread_priority) {
                    Ok(()) => true,
                    Err(error) => {
                        self.show_info(
                            "Runtime settings",
                            &format!("Daemon priority update failed: {error}"),
                        );
                        false
                    }
                };
            let daemon_tolerance_updated =
                match Self::set_daemon_runtime_deadline_tolerance(self.deadline_tolerance_ns) {
                    Ok(()) => true,
                    Err(error) => {
                        self.show_info(
                            "Runtime settings",
                            &format!("Daemon deadline tolerance update failed: {error}"),
                        );
                        false
                    }
                };
            ...
            if apply_clicked
                && !save_clicked
                && daemon_period_updated
                && daemon_priority_updated
                && daemon_tolerance_updated
            {
        ...
        if save_clicked {
            ...
            {
                Ok(crate::gui::tool_model::workspace::RuntimeSettingsSaveTarget::Defaults) => {
                    self.show_info("Runtime settings", "Default values saved");
                }
                Ok(crate::gui::tool_model::workspace::RuntimeSettingsSaveTarget::Workspace) => {
    ...
    fn set_daemon_runtime_priority(priority: i32) -> Result<(), String> {
        let client = rtsyn_ui::api::ApiClient::default();
        match client.set_runtime_priority(priority) {
            Ok(response) if (200..300).contains(&response.status) => Ok(()),
            Ok(response) if response.status == 404 => {
                rtsyn_ui::daemon::DaemonController::default_for_api(client.base_url())
                    .start()
    ...
    fn set_daemon_runtime_deadline_tolerance(tolerance_ns: u64) -> Result<(), String> {
        let client = rtsyn_ui::api::ApiClient::default();
        match client.set_runtime_deadline_tolerance(tolerance_ns) {
            Ok(response) if (200..300).contains(&response.status) => Ok(()),
            Ok(response) if response.status == 404 => {
                rtsyn_ui::daemon::DaemonController::default_for_api(client.base_url())
                    .start()
                    .map_err(|error| format!("daemon restart failed: {error}"))?;
                let retry = client
                    .set_runtime_deadline_tolerance(tolerance_ns)
                    .map_err(|error| error.to_string())?;
                if (200..300).contains(&retry.status) {
                    Ok(())
                } else {
