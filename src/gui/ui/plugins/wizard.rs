...
use super::*;
use crate::gui::daemon_bridge::plugin_creator::{
...
impl GuiApp {
    ...
    pub(super) fn open_install_dialog(&mut self) {
        ...
        crate::gui::spawn_file_dialog_thread(move || {
            let folders = if crate::gui::has_rt_capabilities() {
    ...
    pub(super) fn create_plugin_from_specs(
        ...
        let variables = vars
            ...
            .map(|(name, ty)| {
                ...
                crate::gui::daemon_bridge::plugin_creator::parse_variable_line(&format!(
                    "{name}:{ty}={default}"
                ))
            })
