...
use super::*;
use crate::gui::utils::{format_f64_with_input, normalize_numeric_input, parse_f64_input};
use crate::gui::HighlightMode;
use crate::gui::{
    has_rt_capabilities, kdialog_available, kdialog_file_dialog, spawn_file_dialog_thread,
    save_file_dialog, zenity_available, zenity_file_dialog, LivePlotter,
};
...
fn comedi_device_paths(current: &str) -> Vec<String> {
    let mut paths = std::fs::read_dir("/dev")
        ...
        .filter_map(|entry| {
            ...
            name.strip_prefix("comedi")
                .is_some_and(|index| !index.is_empty() && index.chars().all(|ch| ch.is_ascii_digit()))
                .then(|| format!("/dev/{name}"))
        })
        .collect::<Vec<_>>();
    paths.sort();
    if !current.is_empty() && !paths.iter().any(|path| path == current) {
        paths.insert(0, current.to_string());
    }
    if paths.is_empty() {
        paths.push("/dev/comedi0".to_string());
    }
    paths
}
    ...
    let Some(applied) = applied.as_object_mut() else {
        *applied = Value::Object(serde_json::Map::new());
        ...
        for (_, name, value, value_type) in params {
            applied.insert(name.clone(), runtime_param_value_json(*value_type, value));
        }
        return;
    };
    for (_, name, value, value_type) in params {
...
fn runtime_param_value_json(value_type: ValueType, value: &str) -> Value {
    match value_type {
...
fn runtime_param_text(value: &Value, value_type: ValueType) -> String {
    match value_type {
...
impl GuiApp {
    ...
    pub(crate) fn render_plugin_cards_filtered(
        ...
        let metadata_by_kind: HashMap<String, Vec<(String, f64)>> = self
            ...
            .collect();
        let ui_schema_by_kind: HashMap<String, crate::gui::tool_api::ui::UISchema> = self
            .plugin_manager
            .installed_plugins
            .iter()
            .filter_map(|plugin| {
                plugin
                    .ui_schema
                    .clone()
                    .map(|schema| (plugin.manifest.kind.clone(), schema))
            })
            .collect();
        ...
        let mut pending_param_apply: Vec<(u64, Vec<(u32, String, String, ValueType)>)> = Vec::new();
        ...
        for idx in 0..plugin_count {
            ...
            let opens_plotter_window = opens_external_window
                && crate::gui::DEDICATED_PLOTTER_VIEW_KINDS.contains(&plugin.kind.as_str());
                    ...
                    frame.show(ui, |ui| {
                        ui.vertical(|ui| {
                            ...
                            let (header_rect, _) = ui.allocate_exact_size(
                                egui::vec2(Self::PLUGIN_CARD_WIDTH, 24.0),
                                egui::Sense::hover(),
                            );
                            ...
                            let close_rect = egui::Rect::from_min_size(
                                egui::pos2(header_rect.right() - 12.0, header_rect.top() + 2.0),
                                egui::vec2(20.0, 20.0),
                            );
                            let title_rect = egui::Rect::from_min_max(
                                egui::pos2(id_rect.right() + 8.0, header_rect.top()),
                                egui::pos2(close_rect.left() - 8.0, header_rect.bottom()),
                            );

                            ui.painter().rect_filled(id_rect, 8.0, egui::Color32::from_gray(60));
                            ui.painter().text(
                                id_rect.center(),
                            ...
                            let close_resp = ui.interact(
                                close_rect,
                                ui.id().with(("remove_plugin", plugin.id)),
                                egui::Sense::click(),
                            );
                                                    ...
                                                    for field in &schema.fields {
                                                        ...
                                                        let default_value = field.default.clone().unwrap_or_else(|| {
                                                            match &field.field_type {
                                                                ...
                                                                crate::gui::tool_api::ui::FieldType::Boolean => {
                                                                    Value::from(false)
                                                                }
                                                                crate::gui::tool_api::ui::FieldType::Choice { options } => {
                                                                    Value::from(
                                                                        options
                                                                            .first()
                                                                            .cloned()
                                                                ...
                                                                crate::gui::tool_api::ui::FieldType::DynamicList { .. } => {
                                                                    Value::Array(Vec::new())
                                                                }
                                                            }
                                                        });
                                                                ...
                                                                if let Some(value) = map.get_mut(&key) {
                                                                    kv_row_wrapped(
                                                                        ...
                                                                        |ui| match &field.field_type {
                                                                            crate::gui::tool_api::ui::FieldType::Choice {
                                                                                options,
                                                                            } => {
                                                                            ...
                                                                            crate::gui::tool_api::ui::FieldType::FilePath {
                                                                                ...
                                                                                let mut text = value
                                                                                    ...
                                                                                    .to_string();
                                                                                if key == "device_path" {
                                                                                    let paths = comedi_device_paths(&text);
                                                                                    ...
                                                                                    egui::ComboBox::from_id_source((plugin.id, key, "comedi_device"))
                                                                                        .selected_text(&selected)
                                                                                        .show_ui(ui, |ui| {
                                                                                            for path in paths {
                                                                                                ui.selectable_value(&mut next, path.clone(), path);
                                                                                            }
                                                                                        });
                                                                                    if next != text {
                                                                                    ...
                                                                                    ui.horizontal(|ui| {
                                                                                        ...
                                                                                        if ui.button("Browse").clicked() {
                                                                                            self.runtime_param_path_target =
                                                                                                Some((plugin.id, key.clone()));
                                                                                            let (tx, rx) = mpsc::channel();
                                                                                            self.file_dialogs.runtime_param_path_dialog_rx = Some(rx);
                                                                                            spawn_file_dialog_thread(move || {
                                                                                                let file = if has_rt_capabilities() && zenity_available() {
                                                                                                    zenity_file_dialog("open", None)
                                                                                                } else if kdialog_available() {
                                                                                                    kdialog_file_dialog(None)
                                                                                                } else {
                                                        ...
                                                        for var_name in &vars {
                                                            ...
                                                            if let Ok(Some(value)) = rx.recv() {
                                                                ...
                                                                let is_filepath = field_info
                                                                    .map(|field| matches!(field.field_type, crate::gui::tool_api::ui::FieldType::FilePath { .. }))
                                                                    .unwrap_or(false);
                                                                    ...
                                                                    match &value {
                                                                            ...
                                                                            if is_filepath {
                                                                                ...
                                                                                ui.vertical(|ui| {
                                                                                    ...
                                                                                    if ui.add_sized([value_w, 0.0], egui::Button::new("Browse")).clicked() {
                                                                                        ...
                                                                                        spawn_file_dialog_thread(move || {
                                                                                            let file = save_file_dialog("CSV files", &["csv"], None);
                                                                                            let _ = tx.send(file);
                                                                                        });
                                                                                    }
                                                                                });
                                                                            } else if let Some(field) = field_info {
                                                                                if let crate::gui::tool_api::ui::FieldType::Choice { options } = &field.field_type {
                                                                                    let mut changed = false;
                                                                        ...
                                                                        Value::Number(n) => {
                                                                            ...
                                                                            if let Some(field) = field_info {
                                                                                match &field.field_type {
                                                                                    crate::gui::tool_api::ui::FieldType::Integer { min, max, step } => {
                                                                                        let min = *min;
                                                                                        let max = *max;
                                                                                    ...
                                                                                    crate::gui::tool_api::ui::FieldType::Float { min, max, step } => {
                                                                                        let min = *min;
                                                                                        let max = *max;
                                                                        ...
                                                                        Value::Array(arr) => {
                                                                            if let Some(field) = field_info {
                                                                                if let crate::gui::tool_api::ui::FieldType::DynamicList { item_type, add_label } = &field.field_type {
                                                ...
                                                if !parsed_schema.variables.is_empty() {
                                                    ...
                                                    .show(ui, |ui| {
                                                        ...
                                                        for entry in &parsed_schema.variables {
                                                            ...
                                                            let mut state_label = entry.label.clone();
                                                            if plugin.kind == "comedi" {
                                                                match entry.label.as_str() {
                                                                    "hardware_ready" => {
                                                                        state_label = "Device status".to_string();
                                                                        value_text = if value_text == "0" {
                                                                    ...
                                                                    "last_error" => {
                                                                        state_label = "Device error".to_string();
                                                                        value_text = match value_text.as_str() {
                                                                            "0" => "None",
                                                                            "1" => "Open failed",
                                                                            "2" => "Subdevice unavailable",
                                                                            "3" => "Invalid range",
                                                                            "4" => "Read failed",
                                                                            "5" => "Write failed",
                                                                            _ => "Unknown error",
                                                            ...
                                                            kv_row_wrapped(ui, &state_label, 140.0, |ui| {
                                                                ui.add_enabled_ui(false, |ui| {
                                                                    ui.add_sized(
                                                                        [80.0, 0.0],
                                                                        egui::TextEdit::singleline(
                                        ...
                                        if behavior.supports_apply {
                                            if styled_button(ui, "Modify").clicked() {
                                                ...
                                                if is_api_managed {
                                                    ...
                                                    if let Some(schema) = ui_schema_by_kind.get(&plugin.kind) {
                                                        for (param_id, field) in
                                                            schema.fields.iter().enumerate()
                                                        {
                                                            ...
                                                            values.push((
                                                                param_id as u32,
                                                                field.key.clone(),
                                                                runtime_param_text(&value, value_type),
                                                                value_type,
                                                            ));
                                                        }
                                                    } else {
                ...
                Ok(response) if (200..300).contains(&response.status) => {
                    if running {
                            ...
                            .find(|p| p.id == plugin_id)
                        {
                            let mut values = Vec::new();
                            if let Some(schema) = ui_schema_by_kind.get(&plugin.kind) {
                                for (param_id, field) in schema.fields.iter().enumerate() {
                                    let value_type = value_type_for_field(field);
                                    let value = plugin
                                        .config
                                        .get(&field.key)
                                        ...
                                        .or_else(|| field.default.clone())
                                        .unwrap_or_else(|| {
                                            runtime_param_value_json(value_type, "")
                                        });
                                    values.push((
                                        param_id as u32,
                                        field.key.clone(),
