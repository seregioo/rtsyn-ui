...
use crate::gui::{spawn_file_dialog_thread, BuildAction, PluginFieldDraft};
...
impl GuiApp {
    ...
    pub(crate) fn render_plugins_window(&mut self, ctx: &egui::Context) {
        ...
        let mut module_action: Option<(NodeKind, String, bool)> = None;
                                ...
                                RuntimeNodeDialogMode::AddNode => {
                                        ...
                                        selected.and_then(|idx| entries.get(idx))
                                    {
                                        ...
                                        let add_response = ui.add_enabled(
                                            ...
                                            egui::Button::new(format!("Add {target_name} node")),
                                        );
                                        if add_response.clicked() {
                                            self.queue_runtime_node_add(
                                                target_kind,
                                                descriptor_name,
                                            );
        ...
        if let Some((kind, module_path, delete)) = module_action {
            if delete {
                self.forget_runtime_node_module(kind, &module_path);
            } else {
                self.recompile_runtime_node_module(kind, &module_path);
            }
        }
    ...
    fn start_runtime_module_browse(&mut self, kind: NodeKind) {
        ...
        spawn_file_dialog_thread(move || {
            let selection = if crate::gui::has_rt_capabilities()
                && crate::gui::zenity_available()
            {
                crate::gui::zenity_file_dialog("open", Some("xmake.lua"))
            } else if crate::gui::kdialog_available() {
                crate::gui::kdialog_file_dialog(Some("xmake.lua|RTSyn module (xmake.lua)"))
            } else {
                rfd::FileDialog::new()
    ...
    fn runtime_node_dialog_entries(
        ...
        let loaded_descriptors = match target {
        ...
        for value in loaded_descriptors {
