use crate::api::{ApiClient, NodeKind, NodeState, ValueType};
use crate::daemon::{DaemonConfig, DaemonController};
...
pub fn send_request_to(_path: &str, request: &DaemonRequest) -> Result<DaemonResponse, String> {
    ...
    match request {
        ...
        DaemonRequest::ConnectionRemoveIndex { index } => map_api(
            client.remove_connection((*index + 1) as u32),
            "connection removed",
        ),
        ...
        DaemonRequest::RuntimeSetVariables { id, json } => {
            ...
            if let Some(map) = values.as_object() {
                for (name, value) in map {
                    ...
                    let _ =
                        client.set_param(*id as u32, stable_port_id(name), value_type, &rendered);
                }
            }
        ...
        DaemonRequest::RuntimeSettingsShow => Ok(DaemonResponse::RuntimeSettings {
            settings: crate::gui::workspace_model::WorkspaceSettings::default(),
        }),
        ...
        | DaemonRequest::RuntimeSettingsRestore => Ok(DaemonResponse::Ok {
            message: "settings accepted by gui daemon bridge".to_string(),
        }),
...
fn handle_plugin_request(
    ...
    match request {
        ...
        PluginRequest::PluginAdd { name } => {
            map_api(client.add_node(NodeKind::Plugin, name), "plugin added").map(|_| {
                DaemonResponse::PluginAdded {
                    id: stable_port_id(name) as u64,
                }
            })
        }
        ...
        | PluginRequest::PluginRebuild { name } => Ok(DaemonResponse::Ok {
            message: format!("plugin `{name}` handled by gui daemon bridge"),
        }),
    }
}
...
fn map_api(
    response: crate::Result<crate::api::ApiResponse>,
    message: &str,
) -> Result<DaemonResponse, String> {
    match response {
