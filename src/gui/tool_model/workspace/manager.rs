use crate::gui::tool_model::validation::Validator;
use crate::gui::tool_model::workspace::{io::*, settings::*};
use crate::gui::workspace_model::{WorkspaceDefinition, WorkspaceSettings};
...
impl WorkspaceManager {
    ...
    fn load_or_create_runtime_settings(
        ...
        // Factory settings belong to this version of the application. Do not let a
        // stale generated file override newly introduced or corrected defaults.
        let factory = builtin.clone();
        let _ = save_runtime_settings_file(&factory_path, &factory);

        let defaults_are_current = std::fs::read(&defaults_path)
            .ok()
            .and_then(|data| serde_json::from_slice::<serde_json::Value>(&data).ok())
            .and_then(|value| value.as_object().cloned())
            .is_some_and(|object| {
                object.contains_key("thread_priority")
                    && object.contains_key("deadline_tolerance_ns")
            });
        let defaults = match load_runtime_settings_file(&defaults_path) {
            Ok(settings) if defaults_are_current => settings,
            Ok(_) => {
    ...
    pub fn apply_runtime_settings_patch(
        ...
        if let Some(value) = obj.get("thread_priority") {
            let priority = value
                .as_i64()
                .ok_or_else(|| "thread_priority must be a number".to_string())?;
            settings.thread_priority = (priority as i32).clamp(0, 99);
        }
        if let Some(value) = obj.get("deadline_tolerance_ns") {
            settings.deadline_tolerance_ns = value
                .as_u64()
                .ok_or_else(|| "deadline_tolerance_ns must be a number".to_string())?;
        }

        Validator::normalize_cores(&mut settings.selected_cores);
        settings.thread_priority = settings.thread_priority.clamp(0, 99);
