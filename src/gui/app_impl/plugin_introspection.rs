use crate::gui::tool_model::plugin::plugin_display_name as core_plugin_display_name;
    ...
    pub(crate) fn is_extendable_inputs(&self, kind: &str) -> bool {
        if let Some(cached) = self.behavior_manager.cached_behaviors.get(kind) {
            return matches!(
                ...
                crate::gui::tool_api::ui::ExtendableInputs::Auto { .. }
                    | crate::gui::tool_api::ui::ExtendableInputs::Manual
            );
        }
        crate::gui::tool_model::plugin::is_extendable_inputs(kind)
    }
