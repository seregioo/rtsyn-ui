use crate::gui::state::WorkspaceDialogMode;
use crate::gui::workspace_model::WorkspaceDefinition;
use crate::gui::{spawn_file_dialog_thread, GuiApp, HighlightMode};
use std::fs;
...
impl GuiApp {
            ...
            Ok(()) => {
                ...
                for plugin in &self.workspace_manager.workspace.plugins {
                    ...
                    let _ = self.state_sync.logic_tx.send(
                        crate::gui::runtime_bridge::LogicMessage::SetPluginRunning(
                            plugin.id,
                            plugin.running,
                        ),
                    );
    ...
    pub(crate) fn export_workspace_path(&mut self, source: &Path) {
        ...
        spawn_file_dialog_thread(move || {
            let dest = if crate::gui::has_rt_capabilities() {
                let filename = if !workspace_name.is_empty() {
                    Some(workspace_name.as_str())
                } else {
                    None
                };
                crate::gui::zenity_file_dialog_with_name("save", None, filename)
            } else {
