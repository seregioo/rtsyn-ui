...
mod tests {
    ...
    #[test]
    fn parses_valid_plugin_ui_metadata() {
        // Exercise the parser's format without coupling this crate's unit
        // tests to any particular sibling plugin repository.
        let contents = include_str!("../tests/fixtures/plugin-ui.toml");
        let metadata = PluginUiMetadata::parse(contents).expect("valid plugin UI metadata");

        assert_eq!(metadata.name, "Example device");
        assert_eq!(metadata.controls.len(), 2);
        assert_eq!(metadata.controls[0].name, "device_path");
        assert_eq!(metadata.controls[1].name, "gain");
    }
}
