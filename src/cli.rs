...
use crate::api::{ApiClient, GlobalCommand, NodeKind, ValueType, parse_node_state};
use crate::daemon::{DaemonController, DaemonStatus};
use crate::workspace::Workspace;
use crate::{DEFAULT_API_BASE_URL, Error, Result};
    ...
    let response = match command {
        ...
        CliCommand::ConfigureCsvTelemetry {
            ...
            let values = value_ids
                .iter()
                .map(|value_id| crate::api::CsvValueSelector {
                    node_id: u32::MAX,
                    value_id: *value_id,
                    kind: crate::api::CsvValueKind::Port,
                })
                .collect::<Vec<_>>();
            let mut columns = names;
            columns.extend(measurement_fields.iter().cloned());
            client.configure_csv_telemetry_file(&path, &columns, &values, &measurement_fields)?
        }
...
enum TelemetryCommand {
    ...
    Csv {
        ...
        #[arg(
            long = "measurement-fields",
            value_delimiter = ',',
            num_args = 1..,
            value_parser = parse_csv_measurement_field,
            help = "Comma-separated per-cycle timing fields to append (e.g. latency_ns,actual_period_ns)"
        )]
        measurement_fields: Vec<String>,
    },
}
...
fn parse_csv_measurement_field(value: &str) -> std::result::Result<String, String> {
    const FIELDS: &[&str] = &[
        "period_ns",
        "actual_period_ns",
        "latency_ns",
    ...
    if FIELDS.contains(&value) {
        ...
        Err(format!("unsupported measurement field `{value}`"))
    }
}
