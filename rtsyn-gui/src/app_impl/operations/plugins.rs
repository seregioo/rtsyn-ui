...
use crate::utils::format_f64_with_input;
use crate::GuiApp;
use crate::HighlightMode;
use crate::ViewMode;
use rtsyn_core::plugin::{InstalledPlugin, PluginManager, PluginManifest, PluginMetadataSource};
...
use rtsyn_ui::module::build_runtime_module;
...
impl GuiApp {
    ...
    fn load_runtime_node_module(&mut self, kind: NodeKind, module_path: &str) {
        ...
        let build = match build_runtime_module(module_path) {
            Ok(build) => build,
            Err(error) => {
                self.status = format!("Build {label} failed: {error}");
                self.show_info("Runtime node", &self.status.clone());
                return;
            }
        };
        let shared_library_text = build.shared_library.to_string_lossy().to_string();

        match load_node_with_daemon_retry(kind, &shared_library_text) {
            Ok(response) if (200..300).contains(&response.status) => {
                if let Some(metadata) = runtime_node_metadata_from_response(&response) {
                    ...
                    self.register_runtime_module_metadata(
                        kind,
                        metadata,
                        build.module_root.clone(),
                        Some(build.shared_library.clone()),
                    );
                    self.set_runtime_node_name_if_empty_or_inferred(kind, &node_name);
                    self.remember_runtime_node_name(kind, &node_name);
                }
                self.status = format!("{label} module loaded: {shared_library_text}");
                self.remember_runtime_module(kind, &shared_library_text);
    ...
    pub(crate) fn add_runtime_plugin_node(&mut self, node_name: &str) {
        self.add_runtime_node(NodeKind::Plugin, node_name);
    }

    pub(crate) fn add_runtime_device_node(&mut self, node_name: &str) {
        self.add_runtime_node(NodeKind::Device, node_name);
    }

    fn add_runtime_node(&mut self, kind: NodeKind, node_name: &str) {
        let node_name = node_name.trim();
        let label = runtime_node_kind_label(kind);
        if node_name.is_empty() {
            match self.reload_runtime_node_module_and_add(kind, "") {
                Some(Ok(response)) if (200..300).contains(&response.status) => {
                    self.apply_runtime_node_added(kind, "", &response, label);
                    return;
                }
                Some(Ok(response)) => {
                    self.status = format!("Add {label} failed: HTTP {}", response.status);
                    self.show_info(
                        "Runtime node",
                        &format!("{} {}", self.status, response.body),
                    );
                ...
                Some(Err(error)) => {
                    self.status = format!("Add {label} failed: {error}");
                    self.show_info("Runtime node", &self.status.clone());
                    return;
                }
                None => {
        ...
        match add_node_with_daemon_retry(kind, node_name) {
            Ok(response) if (200..300).contains(&response.status) => {
                self.apply_runtime_node_added(kind, node_name, &response, label);
            }
            Ok(response) => {
                if response_is_unknown_node(&response) {
                    if let Some(retry) = self.reload_runtime_node_module_and_add(kind, node_name) {
                        match retry {
                            Ok(response) if (200..300).contains(&response.status) => {
                                self.apply_runtime_node_added(kind, node_name, &response, label);
                                return;
                            ...
                            Ok(response) => {
                                self.status = format!(
                                    "Add {label} failed after reload: HTTP {}",
                                    response.status
                                );
                                self.show_info(
                                    "Runtime node",
                            ...
                            Err(error) => {
                                self.status = format!("Add {label} failed after reload: {error}");
                                self.show_info("Runtime node", &self.status.clone());
                                return;
                            }
                        }
        ...
        match load_node_with_daemon_retry(kind, &shared_library_text) {
            Ok(response) if (200..300).contains(&response.status) => {
                if let Some(metadata) = runtime_node_metadata_from_response(&response) {
                    ...
                    Some(add_node_with_daemon_retry(kind, node_name))
                }
            }
    ...
    fn apply_runtime_node_added(
        ...
        self.workspace_manager
            .workspace
            .plugins
            .push(workspace::PluginDefinition {
                id,
    ...
    pub(crate) fn start_all_plugins(&mut self) {
        ...
        for plugin in &mut self.workspace_manager.workspace.plugins {
            if !plugin.running {
                ...
                for (param_id, _, value) in &values {
                    match client.set_param(node_id, *param_id, ValueType::F64, value) {
                        Ok(response) if (200..300).contains(&response.status) => {}
                        Ok(response) => {
...
fn load_node_with_daemon_retry(
    ...
    let client = ApiClient::default();
    let response = client.load_node(kind, shared_library_path)?;
    if response.status != 404 {
        return Ok(response);
    }

    restart_default_daemon()?;
    ApiClient::default().load_node(kind, shared_library_path)
}
...
mod tests {
    use super::{
        canonical_runtime_node_kind_for, mask_from_display_entries, response_is_unknown_node,
        runtime_module_descriptor_name, runtime_node_metadata_from_response,
        runtime_node_result_from_response, workspace_node_has_runtime_descriptor,
    };
    use rtsyn_core::plugin::{InstalledPlugin, PluginManifest};
    use rtsyn_ui::api::ApiResponse;
    use rtsyn_ui::metadata::PluginUiMetadata;
