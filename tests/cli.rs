...
use rtsyn_ui::cli::{CliCommand, CliOptions, execute};
...
fn parses_csv_telemetry_command() {
    ...
    assert_eq!(
        ...
        CliCommand::ConfigureCsvTelemetry {
            ...
            value_ids: vec![4, 8],
            measurement_fields: vec![],
        }
    );
}

#[test]
fn parses_csv_telemetry_measurement_fields() {
    let csv = CliOptions::parse([
        "telemetry".to_string(),
        "csv".to_string(),
        "/tmp/rtsyn-values.csv".to_string(),
        "vm:2".to_string(),
        "ai0:10".to_string(),
        "--measurement-fields".to_string(),
        "period_ns,actual_period_ns,latency_ns,wake_lateness_ns,skipped_cycle_count,missed_cycle,deadline_missed,devices_read_ns,plugins_time_ns,devices_write_ns".to_string(),
    ])
    .unwrap();
    ...
    assert_eq!(
        ...
        CliCommand::ConfigureCsvTelemetry {
            path: "/tmp/rtsyn-values.csv".to_string(),
            names: vec!["vm".to_string(), "ai0".to_string()],
            value_ids: vec![2, 10],
            measurement_fields: vec![
                "period_ns",
                "actual_period_ns",
...
#[test]
fn rejects_unknown_csv_measurement_field() {
    assert!(
        CliOptions::parse([
            "telemetry".to_string(),
            "csv".to_string(),
...
fn execute_csv_telemetry_checks_daemon_and_posts_config() {
    let requests = capture_execute_requests(|client| {
        let output = execute(
            ...
            CliCommand::ConfigureCsvTelemetry {
                ...
                names: vec!["left".to_string(), "right".to_string()],
                value_ids: vec![4, 8],
                measurement_fields: vec!["latency_ns".to_string(), "actual_period_ns".to_string()],
            },
        )
        .unwrap();
        assert!(output.contains("accepted"));
    ...
    assert!(
        requests[1].contains("\"names\":[\"left\",\"right\",\"latency_ns\",\"actual_period_ns\"]")
    );
    assert!(requests[1].contains("\"measurement_fields\":[\"latency_ns\",\"actual_period_ns\"]"));
